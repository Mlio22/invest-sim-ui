...
export default function RegisterForm() {
  ...
  const oauthProviders = [
    { key: "google", label: t.register.googleBtn, icon: <GoogleIcon /> },
    { key: "microsoft", label: t.register.microsoftBtn, icon: <MicrosoftIcon /> },
    { key: "facebook", label: t.register.facebookBtn, icon: <FacebookIcon /> },
    { key: "twitter", label: t.register.twitterBtn, icon: <XIcon />, dark: true },
  ];
    ...
    <div className="flex flex-col min-h-screen px-8 py-12 lg:px-14 max-w-lg mx-auto w-full">
      ...
      <div className="mb-8">
        <h1 className="text-4xl lg:text-5xl font-extrabold text-white leading-tight mb-2">
          {t.register.title}
          <br />
          <span className="text-[#f97316]">{t.register.appName}</span>
        </h1>
        <p className="text-white/55 text-base">{t.register.subtitle}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3 mb-5">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#f97316] hover:bg-[#ea6c0a] active:bg-[#dc5f00] disabled:opacity-60 text-white font-bold text-sm rounded-lg py-3.5 transition-colors mt-1"
        >
          {loading ? "…" : t.register.signUpBtn}
        </button>
      </form>
